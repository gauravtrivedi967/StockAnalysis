...
def real_time_price(stock_code):
    ...
    try:
        ...
        price,volume,one_year_target=[], [], []
    return price,volume,one_year_target
...
while(True):
    info=[]
    col=[]
    time_stamp=datetime.datetime.now() - datetime.timedelta(hours=18)
    time_stamp=time_stamp.strftime('%Y-%m-%d %H:%M:%S')
    for stock_code in Stock:
        stock_price,volume,one_year_target=real_time_price(stock_code)
